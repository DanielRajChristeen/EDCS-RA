...
void attachAll() {
  ...
  baseServo.write(baseM.current);
  armServo.write(armM.current);
  elbowServo.write(elbowM.current);
  gripServo.write(gripM.current);

  servosAttached = true;
  Serial.println("[CONTROL] Servos ATTACHED @ 50Hz");
}
...
void setup() {
  ...
  Serial.println("[SYSTEM] ARM 2 READY");
}
